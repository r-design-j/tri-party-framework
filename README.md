...
- 原生 PowerShell 当前只建议做环境准备、检查、quickstart、安装 dry-run、安装计划、prompt、guide、validate-run、evidence-template、evidence-fill、package 准备，以及 WSL2 bootstrap：`winget install Git.Git`、`winget install Python.Python.3.12`、`.\scripts\agentparty.ps1 install --pack triparty --target-os windows_powershell`、`.\scripts\agentparty-windows-wsl2-bootstrap.ps1 -Execute`。bootstrap 会把真实安装交给 WSL2 内的 Linux 路径执行，不代表 native PowerShell parity。
...
- Windows 用户可以复制 `.\scripts\agentparty-windows-wsl2-bootstrap.ps1 -Execute` 作为一条 PowerShell 入口；它是 WSL2 bootstrap，不是 native PowerShell `run` 或 `install --execute`。
